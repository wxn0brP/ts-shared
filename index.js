...
export class JwtManager {
    ...
    secretKey;
    keyIndex;
    constructor(keyIndex = null, secretKey = process.env.JWT) {
        this.keyIndex = keyIndex || KeyIndex;
        this.secretKey = new TextEncoder().encode(secretKey);
    }
    async init(db) {
        this.keyManager = new KeyManager(db);
        await this.keyManager.initKeyPairs(this.keyIndex);
    }
    /**
    ...
    */
    async create(data, exp = true, encrypt = false) {
     ...
     */
    async decode(token, keyIndex = false) {
        try {
            if (keyIndex !== false) {
